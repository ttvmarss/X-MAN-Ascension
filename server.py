...
- [ACTION:OPEN_APP] app_name — open any app or game on the user's PC. Examples: Steam, Discord, Spotify, Roblox, FeatherClient, TikTok LIVE Studio, Epic Games, Chrome, Edge, Notepad, Calculator.
  "open Steam" → [ACTION:OPEN_APP] Steam
  "launch Discord" → [ACTION:OPEN_APP] Discord
  "open Epic Games" → [ACTION:OPEN_APP] Epic Games
  "open TikTok Live Studio" → [ACTION:OPEN_APP] TikTok LIVE Studio
  "open Feather Client" → [ACTION:OPEN_APP] FeatherClient
- [ACTION:CLOSE_APP] app_name — close/quit any running app or game.
  "close Chrome" → [ACTION:CLOSE_APP] Chrome
  "quit Discord" → [ACTION:CLOSE_APP] Discord
  "close Steam" → [ACTION:CLOSE_APP] Steam
  "shut down Spotify" → [ACTION:CLOSE_APP] Spotify
    ...
    try:
        response = await client.messages.create(
            ...
            system=(
                ...
                "Return ONLY valid JSON: {\"action\": \"open_terminal|browse|open_app|close_app|build|chat\", "
                "\"target\": \"name of app or URL\"}\n"
                "open_terminal = user wants to open terminal or command prompt\n"
                "browse = user wants to search the web, visit a URL, or open a website like YouTube, Netflix, Twitch\n"
                "open_app = user wants to OPEN/LAUNCH an application or game (Steam, Discord, Spotify, Epic Games, FeatherClient, TikTok LIVE Studio, Chrome, Edge, etc.)\n"
                "close_app = user wants to CLOSE/QUIT/KILL an application or game\n"
                ...
                "If user says 'close Chrome', 'quit Discord', 'kill Steam', 'shut down Spotify' use close_app.\n"
...
async def _execute_close_app(app_name: str):
    """Execute a close-app action from an LLM-embedded [ACTION:CLOSE_APP] tag."""
    try:
        from actions import close_app
        result = await close_app(app_name)
        log.info(f"Close app '{app_name}': {result}")
    except Exception as e:
        log.error(f"Close app failed: {e}")
